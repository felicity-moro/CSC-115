public class A6Tester {	
	...
	public static void eventLineTests() {
		...
		try {
			...
			expected = p3;
			if (result == null) {
				displayResults(false, "handled first person in line 3 a");
				System.out.println(result);
			} else {
				displayResults(result.equals(expected), "handled first person in line 3 b");
			}
		} catch (Exception e) {
		...
		try {
			inserted = eLine.premiumEntry(p7, 0);
			displayResults(inserted==true, "inserted into specific pos");
		} catch (Exception e) {
			displayResults(false, "inserted into specific pos");
		}
