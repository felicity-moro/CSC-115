...
public class Attendee implements Comparable<Attendee>{
	...
	public int compareTo(Attendee other) {
		...
		if (getEventTicket().getTicketType() == (other.getEventTicket().getTicketType())){
			if (getArrivalTime().compareTo(other.getArrivalTime()) < 0){
				return -1;
			}else if(getArrivalTime().compareTo(other.getArrivalTime()) == 0){
				return 0;
			}else{
				return 1;
			}
		}
		else if (getEventTicket().getTicketType() > (other.getEventTicket().getTicketType())){
			return 1;
		}
		return -1;
	}
