...
public class Lab7Tester {
	...
	public static void testBasicStack() {
		...
		try{
			s.pop();
			displayResults(false, "LAB TEST should be empty, exception not thrown");
		}catch(StackEmptyException see){
			displayResults(true, "LAB TEST should be empty, exception is properly thrown");
	 ...
	 */
	public static String reverseString(String str) throws EmptyStackException {
		String result = "";
	...
	public static void testQueue() {
		...
		}catch (QueueEmptyException m){
			displayResults(false, "exception caught when it shouldnt have");
		}
		...
		try{ q = new QueueRefBased<Integer>();
			subtestSize = 10;
			for (int i = 0; i < subtestSize; i++) {
				q.enqueue(i);
			}
		...
		try{
			q.dequeue();
			displayResults(false, "exception not caught when it shouldnt have");
		}catch (QueueEmptyException m){
			displayResults(true, "exception caught when it shouldnt have LAB");
		}
