...
public class IntegerLinkedList {
	...
	public void doubleValuesRec(IntegerNode cur){
		if (cur == null){
			return;
		}else{
			int newData = cur.getData()*2;
			cur.setData(newData);
			doubleValuesRec(cur.next);
		}
	...
	public void doubleOddValuesRec(IntegerNode cur){
		if (cur == null){
			return;
		}else if (cur.getData()%2 != 0){
			int newData = cur.getData()*2;
			cur.setData(newData);
			doubleOddValuesRec(cur.next);
		} else {
			doubleOddValuesRec(cur.next);
		}
	}
	...
	public int product() {
		...
		return productRec(head, 1);
	}

	public int productRec(IntegerNode cur, int total){
		if (cur == null){
			return total;
		} else {
			return productRec(cur.next, total*cur.getData());
		}
	}
	/*
	...
	public boolean allNegativeRec(IntegerNode cur){

		if (cur == null){
			return true;
		} else if (cur.getData() >= 0){
			return false;
		} else {
			return allNegativeRec(cur.next);
		}
	}
	...
	public boolean isSortedAscending() {
		...
		return isSortedAscendingRec(head.next, head.getData());
	}  
	
	public boolean isSortedAscendingRec(IntegerNode cur, int prev){
		if (cur == null){
			return true;
		} else if (cur.getData() < prev){
			return false;
		}else{
			return isSortedAscendingRec(cur.next, cur.getData());
		}
	}
}
